...
export default class Truncate extends Component {
  static propTypes = {
    ...
    breakOn: PropTypes.oneOf(["letter", "word"])
  };
  ...
  calcTargetWidth(callback) {
    ...
    const parentNodeStyle = window.getComputedStyle(target.parentNode);
    const targetWidth = Math.floor(
      target.parentNode.getBoundingClientRect().width -
      parseFloat(parentNodeStyle.getPropertyValue("padding-left")) -
      parseFloat(parentNodeStyle.getPropertyValue("padding-right"))
    );
  ...
  getLines() {
    ...
    if (breakOn === "letter") {
      breakChar = '';
    } else if (breakOn === "word") {
      breakChar = ' ';
    }
    const textLines = text.split('\n').map(line => line.split(breakChar));
    ...
    for (let line = 1; line <= numLines; line++) {
      ...
      let resultLine = textWords.join(breakChar);
      ...
      if (line === numLines) {
        ...
        const textRest = textWords.join(breakChar);
        ...
        while (lower <= upper) {
          ...
          const testLine = textWords.slice(0, middle + 1).join(breakChar);
        ...
        resultLine = textWords.slice(0, lower).join(breakChar);
